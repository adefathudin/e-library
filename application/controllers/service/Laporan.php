...
class Laporan extends REST_Controller {
    ...
    public function create_laporan_post() {
        ...
        $data_laporan = [
            'report_by' => $report_by,
            'tanggal_lapor' => date("Y-m-d h:i:sa"),
    ...
    public function list_laporan_pending_get() {

        $list_laporan_dashboard = $this->laporan_m->get_by(['status_laporan' => PENDING]);

        if ($list_laporan_dashboard) {
            $output['status'] = true;
            $output['item'] = $list_laporan_dashboard;
            $output['total_laporan_pending'] = $this->laporan_m->get_count(['status_laporan' => PENDING]);
        } else {
            $output['status'] = false;            
            $output['item'] = "";
            $output['message'] = $this->laporan_m->get_last_message();
            $output['total_laporan_pending'] = 0;
        }
    ...
    public function update_status_laporan_post() {
        ...
        if ($jenis_status1 == VERIFIKASI || $jenis_status1 == PROSES){
            
            $result_fifo = $this->laporan_m->get_count_fifo($id_laporan,$jenis_status1);
                if ($result_fifo->count > 0){
                    $output['status'] = false;
                    $output['message'] = 'Ada '.$result_fifo->count.' laporan terdahulu yang belum di'.$jenis_status1.'! Harap '.$jenis_status1.' terlebih dahulu!';
                    $this->response($output);
             };
        }
        
        $list = $this->laporan_m->get_by(['status_laporan' => $submit, 'tanggal_'.$submit]);
        ...
        if ($insert_laporan) {
            ...
            $this->notifikasi_m->save([
                'level' => $level,
                'judul' => 'Laporan "' . $nama_laporan . '"',
                'isi' => 'Laporan baru "' . $nama_laporan . '" telah di' . $submit . ''
            ]);

            $output['status'] = true;
            $output['message'] = "Laporan berhasil di $submit";
        } else {
            $output['status'] = false;
            $output['message'] = "Something wrong";
        }
